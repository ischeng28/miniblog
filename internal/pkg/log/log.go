...
// Logger定义了miniblog项目的日志接口，该接口只包含了支持的日志记录方法
...
// zapLogger是Logger接口的具体实现，它底层封装了zap.Logger
type zapLogger struct {
	z *zap.Logger
}

// 确保zapLogger实现了Logger接口，以下变量赋值，可以使错误在编译器被发现
...
// NewLogger根据传入的opts创建Logger
func NewLogger(opts *Options) *zapLogger {
	if opts == nil {
		opts = NewOptions()
	}
	//	将文本格式的日志级别，例如info转换为zapcore.Level类型以供后面使用
	var zapLevel zapcore.Level
	if err := zapLevel.UnmarshalText([]byte(opts.Level)); err != nil {
		//	如果制定了非法的日志级别，则默认使用info级别
